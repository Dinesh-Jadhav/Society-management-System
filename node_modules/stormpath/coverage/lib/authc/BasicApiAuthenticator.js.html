...
&nbsp;
  self.application.getApiKey(self.id, function (err, apiKey) {
    if (err) {
      return callback(err.status === 404 ? new ApiAuthRequestError({userMessage: 'Invalid Client Credentials', error: 'invalid_client',  statusCode: 401}) : <span class="branch-1 cbranch-no" title="branch not covered" >err)</span>;
    }
