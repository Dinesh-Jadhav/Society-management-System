...
socialApp.controller('parkingList', ['$scope', '$http', '$routeParams', 'DTOptionsBuilder', function($scope, $http, $routeParams, DTOptionsBuilder) {
    ...
    $scope.assignParking = [];
    $http.post('/listOfAssignParking', { block_id: block_id }).success(function(response) {
        console.log(response);
        if (response.hasOwnProperty('success')) {
            $scope.assignParking = response.data;
        }
    });
}]);
