...
socialApp.controller('staffFor', ['$scope','$http','$route','$routeParams', function($scope,$http,$route,$routeParams){
    $scope.$emit('LOAD');
    var block_id=atob($routeParams.blockID);
    $scope.staffMemberFor = {};

    $scope.addStaffFor=function(){
        $http.post('/addStaffMemberFor',$scope.staffMemberFor).success(function(response){
            $route.reload();
        });
    }
}]);
